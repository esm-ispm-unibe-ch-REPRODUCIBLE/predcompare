...
#' @export
bencalibr=function (data = NULL, Ngroups = 5, y.observed, treat, predicted.treat.0, 
                    predicted.treat.1, type = "continuous", smoothing.function = "rlm", 
  ...
  if (type == "continuous") {
    ...
    if (!nulldata) {
      ...
      data <- data.frame(y.observed, treat = treat, predicted.treat.0, 
                         predicted.treat.1)
      data$predicted.benefit <- data$predicted.treat.1 - 
        data$predicted.treat.0
    }
    ...
    d1 <- quantile(data$predicted.benefit, probs = seq(0, 
                                                       1, 1/Ngroups))
    g1 <- list()
    for (i in 1:Ngroups) {
  ...
  if (type == "binary") {
    ...
    if (measure == "logor") {
      ...
      for (i in 1:Ngroups) {
        ...
        observed = c(observed, logit(mean(g1[[i]]$y.observed[g1[[i]]$treat == 
                                                               1])) - logit(mean(g1[[i]]$y.observed[g1[[i]]$treat == 
                                                                                                      0])))
        errors.predicted <- c(errors.predicted, sd(g1[[i]]$benefit))
        errors.observed <- c(errors.observed, sqrt(1/sum(g1[[i]]$y.observed[g1[[i]]$treat == 
                                                                              1]) + 1/sum(g1[[i]]$y.observed[g1[[i]]$treat == 
                                                                                                               1] == 0) + 1/sum(g1[[i]]$y.observed[g1[[i]]$treat == 
                                                                                                                                                     0]) + 1/sum(g1[[i]]$y.observed[g1[[i]]$treat == 
                                                                                                                                                                                      0] == 0)))
      }
      dat1 <- data.frame("pred" <- predicted, "obs" <- observed, 
                         "se.pred" <- errors.predicted, "se.obs" <- errors.observed)
      p1 <- ggplot(dat1, aes(x <- pred, y <- obs)) + geom_point(size = 3, 
                                                                shape = 20) + labs(x = "Predicted benefit", 
                                                                                   y = "Observed benefit") + geom_abline(intercept = 0, 
                                                                                                                         slope = 1, color = "black", linetype = "dashed", 
                                                                                                                         size = 0.5) + geom_errorbar(aes(ymin = obs - 
                                                                                                                                                           se.obs, ymax = obs + se.obs), width = 0.1) + 
        geom_errorbarh(aes(xmin = pred - se.pred, xmax = pred + 
                             se.pred)) + geom_smooth(method = "lm", colour = "blue", 
                                                     size = 0.5) + theme(aspect.ratio = 1) + ggtitle("Calibration for benefit (measure: log odds ratios)")
      r1 <- max((suppressMessages(layer_scales(p1))$x$range$range))
      r2 <- min((suppressMessages(layer_scales(p1)$x$range$range)))
      s1 <- max((suppressMessages(layer_scales(p1)$y$range$range)))
      s2 <- min((suppressMessages(layer_scales(p1)$y$range$range)))
    ...
    if (measure == "RD") {
      ...
      data$benefit <- expit(data$predicted.treat.1) - 
        expit(data$predicted.treat.0)
      ...
      for (i in 1:Ngroups) {
        ...
        errors.observed <- c(errors.observed, sqrt(sum(g1[[i]]$y.observed[g1[[i]]$t == 
                                                                            1]) * sum(g1[[i]]$y.observed[g1[[i]]$t == 
                                                                                                           1] == 0)/length(g1[[i]]$y.observed[g1[[i]]$t == 
                                                                                                                                                1])^3 + sum(g1[[i]]$y.observed[g1[[i]]$t == 
                                                                                                                                                                                 0]) * sum(g1[[i]]$y.observed[g1[[i]]$t == 
                                                                                                                                                                                                                0] == 0)/length(g1[[i]]$y.observed[g1[[i]]$t == 
                                                                                                                                                                                                                                                     0])^3))
      }
      dat1 <- data.frame("pred" <- predicted, "obs" <- observed, 
                         "se.pred" <- errors.predicted, "se.obs" <- errors.observed)
      p2 <- ggplot(dat1, aes(x <- pred, y <- obs)) + geom_point(size = 3, 
        ...
        geom_errorbarh(aes(xmin = pred - se.pred, xmax = pred + 
                             se.pred), height = 0.005) + geom_smooth(method = "lm", 
                                                                     colour = "blue", size = 0.5) + theme(aspect.ratio = 1) + 
